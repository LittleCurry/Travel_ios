...
- (void)addStatusAndImage:(NSString *)status imageName:(NSString *)imageName type:(NSString *)type tapViewWithBlock:(tapViewWithBlock)block{
    ...
    bigButton.tag = 10087;
    bigButton.frame = CGRectMake((WIDTH-100)/2, self.view.center.y-100, 100, 100);
    [bigButton addTarget:self action:@selector(tapBlock) forControlEvents:UIControlEventTouchUpInside];
...
- (void)show {
    ...
    imageView.hidden = NO;
    UIButton *bigButton = [self.view viewWithTag:10087];
    bigButton.hidden = NO;
    if (self.tapGestureBlock) {
        [self.view addGestureRecognizer: self.tapGestureBlock];
    }
...
- (void)hide {
    ...
    UIImageView *bigButton = [self.view viewWithTag:10087];
